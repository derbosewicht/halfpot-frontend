...
export default function Component() {
    ...
    try {
      ...
      const response = await fetch('https://polar-ravine-08798.herokuapp.com/purchase', {
        ...
        body: JSON.stringify({
          ...
          potAmount: potAmount,  // Include the potAmount
        }),
      });
      ...
      // If the purchase is successful, update the potAmount and spots in state
      console.log('Sticker purchased successfully');
      setPotAmount(prevAmount => prevAmount + 1);
      setSpots(prevSpots => prevSpots + 1);
    } catch (error) {
      // Log any errors that occurred during the purchase
  ...
  const handleLogin = async () => {
    try {
      ...
      const data = await response.json(); // The response should include a token
  
      console.log("Logged in successfully:", data);
  
      // Update the React state to indicate the user is logged in
      setIsLoggedIn(true);
      setUsername(email);  // Set the username to the logged-in user's email
  ...
  const handleLogout = () => {
    ...
    // Update the React state to reflect that the user is no longer logged in
    setIsLoggedIn(false);
    setUsername('');
  };
